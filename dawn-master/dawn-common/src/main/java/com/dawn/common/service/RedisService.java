...
public class RedisService {
    ...
    private PageResult getPageResult(PageRequest request, String cacheName, Object mapper, String methodName, String field) {
        ...
        if (hExists(cacheName, field)) {
            pageResult = (PageResult) hGet(cacheName, field);
            return pageResult;
        }
        ...
        if (!BeanUtil.isEmpty(pageResult) && !CollUtil.isEmpty(pageResult.getContent())) {
            hPut(cacheName, field, pageResult);
        }
        return pageResult;
    }
     ...
     * @return
     */
    public <T> List<T> getRedisCache(String key,String hashKey) {
        if (hExists(key,hashKey)){
            return (List<T>)hGet(key,hashKey);
        }
        return null;
    }
